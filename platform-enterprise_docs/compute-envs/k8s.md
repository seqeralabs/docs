...
- The **Pod cleanup policy** determines when Seqera deletes the run's Nextflow head pod. **On success** (the default) deletes it when the run succeeds, **Always** deletes it regardless of outcome, and **Never** keeps it. This policy does not delete task pods. See [Kubernetes pods remain after a run ends](../troubleshooting_and_faqs/nextflow#kubernetes-pods-remain-after-a-run-ends).
