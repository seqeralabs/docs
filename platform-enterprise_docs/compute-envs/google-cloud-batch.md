...
2. Set Seqera as an OIDC provider within the pool. Set the Issuer URL to https://{your-platform-domain}/api, where {your-platform-domain} is the hosted URL. The discovery endpoints are at /api/.well-known/openid-configuration and /api/.well-known/jwks.json, and both must be publicly reachable from GCP STS.
