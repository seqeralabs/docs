...
- **Pipeline secrets KMS key**: A customer-managed KMS key (CMK) that encrypts the temporary AWS Secrets Manager secrets Seqera creates for runs that use pipeline secrets. This field accepts a key ARN or a key ID. A key ARN must be in the same region as the compute environment. If unspecified, Seqera uses the installation default set with [`TOWER_AWS_SECRETS_KMS_KEY_ID`](../enterprise/configuration/overview#compute-environments), or the default AWS-managed key if neither is set.

    :::caution
    The role attached to the instance profile reads the secrets at task start. The instance profile that Seqera provisions automatically does not include the KMS permissions required to decrypt secrets encrypted with a customer-managed key. To use this field, specify a custom **Instance Profile** whose role grants the KMS permissions described in [Custom instance profile](#custom-instance-profile).
    :::
...
If the compute environment specifies a **Pipeline secrets KMS key**, the role also requires `kms:GenerateDataKey` and `kms:Decrypt` on that key to create and read secrets encrypted with the key. Grant these actions either by naming the role in the key policy, or in the role's own IAM policy if the key policy delegates to IAM. The default key policy created by `aws kms create-key` delegates to IAM:
