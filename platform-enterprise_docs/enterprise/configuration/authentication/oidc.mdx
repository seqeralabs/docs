...
| `TOWER_OIDC_SCOPES` | Optional. Comma-separated OAuth 2.0 scopes the OIDC client requests at login. Defaults to `openid,email,profile`. `openid` is always included. Add `groups` when your IdP only emits the group claim for a requested scope (see the note below). |
...
:::note
If you plan to use IdP-delegated teams, your OIDC token must include a `groups` claim. Some providers (for example, Okta) only emit this claim when a matching scope is requested. In that case, add `groups` to the requested scopes with `TOWER_OIDC_SCOPES=openid,email,profile,groups`. See [IdP claim mapping](./idp-delegation/claim-mapping) for the per-IdP configuration steps.
:::
