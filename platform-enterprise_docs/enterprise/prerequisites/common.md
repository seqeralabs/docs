...
Configure `skopeo` to access the Seqera container registry, and your internal registry if needed.
...
Listing the available tags for a given image can be done with the `list-tags` command:

```console
skopeo list-tags docker://cr.seqera.io/private/nf-tower-enterprise
```

Syncing images can be done with the `sync` command. For example, to sync all images stored in the `cr.seqera.io/private/nf-tower-enterprise/data-studio/connect-server` repository to your internal registry, run:
...
The above command can be automated with a CI/CD pipeline to periodically sync images.
