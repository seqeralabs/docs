...
These run automatically on every PR and gate the merge.
...
- **[pre-commit-fix.yaml](.github/workflows/pre-commit-fix.yaml)** — triggers when a PR comment starts with `fix formatting`. Re-runs pre-commit, commits any changes, and pushes to the PR branch.
