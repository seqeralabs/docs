...
export default function ProductItem({ hit, components }) {
  return (
    <a 
      href={hit.url} 
      className="aa-ItemLink search-item-link typo-small font-normal px-2 py-2 flex whitespace-pre-wrap text-blu-600"
    >
      <div className="flex flex-row w-full gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-start pb-2 last:pb-0 text-sm text-slate-900 break-words">
        ...
        <div className="shrink-0 text-right">
          ...
          {(hit.url.includes('https://docs.seqera.io/wave') || hit.url.match(/https:\/\/docs\.seqera\.io\/([^\/]+)/)?.[1] === 'wave' || hit.url.includes('https://docs.seqera.io/changelog/tags/wave') || hit.url.includes('https://docs.seqera.io/changelog/wave')) && (
            <div className="flex items-start pb-2 last:pb-0">
              ...
              {hit._highlightResult?.version?.[0]?.value && (
                <span className={`${styles.productItemVersion} text-xs px-2 py-1 bg-slate-100 rounded-md text-slate-900 whitespace-pre-wrap`}>Latest</span>
              )}
            </div>
          )}
