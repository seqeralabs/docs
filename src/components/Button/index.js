...
const Button = ({ children, onClick, href, to, className, sameTab, disabled, ...attributes }) => {
    ...
    if (url) {
        ...
        if (sameTab === false)
            isOutLink = true;
        return (<a href={url} onClick={onClick} className={cn} target={isOutLink ? "_blank" : undefined} rel={isOutLink ? "noreferrer" : undefined} aria-disabled={disabled}>
        {content}
      </a>);
    }
    return (<button type="button" onClick={onClick} className={cn} disabled={disabled}>
      {content}
    </button>);
};
