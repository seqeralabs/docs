    ...
    return (<>
            <form method="get">
                ...
                <div className="relative inline-block w-full whitespace-nowrap sm:w-96">
                    <input id="filter" name="filter" type="text" value={filterValue} onChange={(e) => setFilterValue(e.target.value)} className="my-3 inline-block w-full rounded-lg border border-solid border-gray-500 bg-black/5 p-2 pr-10  focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-black/20 sm:mx-3" placeholder="Tool name, description or keyword"/>
                    {filterValue && (<button onClick={handleClearFilter} id="clear_filter" className="absolute -right-4 top-4 bg-transparent border-0 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300" aria-label="Clear search">
                ...
                <div className="flex flex-col">
                    ...
                    {filteredModules.map((module) => (<div className="module-row grid-cols-4 lg:grid">
                            <a href={"/multiqc/modules/" + module.id.split("/").pop().replace(".md", "")} className=" hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer hover:no-underline">
