...
:::tip
Manual AWS Batch configuration is only necessary if you don't use Tower Forge.

Tower Forge _automatically creates_ the AWS Batch queues required for your workflow executions.
:::
...
1. Select **Amazon EC2** as the compute environment configuration.
    ...
    :::note
    Seqera AWS Batch compute environments created with [Tower Forge](../../compute-envs/aws-batch.md#tower-forge-compute-environment) support using Fargate for the head job, but manual compute environments must use EC2.
    :::
...
Use the AWS resources created on this page to create your [manual AWS Batch compute environment](../../compute-envs/aws-batch.md#manual).
