# Release notes for Seqera Enterprise version 23.4.3

Seqera Platform Enterprise version 23.4.3 introduces a redesigned UI, VM instance template support for Google Cloud Batch, and database deployment improvements. A number of bug fixes and performance enhancements have also been included in this major release.

:::note
Version 23.4.3 is the baseline for the 24.3 major release cycle.
:::
...
The `TOWER_ENABLE_ARM64` configuration environment variable is no longer needed to enable Graviton instance support in AWS Batch compute environments. ARM64 CPU architecture support is enabled by default. 
