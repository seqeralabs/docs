...
- **MySQL database**: An external database such as [Azure Database for MySQL][azure-db-create] is highly recommended for production deployments.

- **SSL certificate**: An SSL certificate is required for your Seqera instance to handle HTTPS traffic. See [SSL/TLS configuration](../configuration/ssl_tls.mdx#azure-deployments-use-application-gateway-for-tls-termination-and-load-balancing) for more information.
...
- **(Optional) DNS**: DNS is required to support human-readable domain names and load-balanced traffic. See [Azure DNS][azure-dns] to learn about domain aquisition and record management.

These decisions must be made before you continue as they impact how Seqera configuration files are updated.

### Prerequisites for Docker

A [Linux VM][azure-linux] instance is required to deploy Seqera Enterprise via Docker Compose. See the [detailed instructions](#azure-setup) to provision a VM instance for this purpose.
...
To customize your cluster's Ingress Controller to support HTTPS redirects and TLS certificates, see [Application Gateway Ingress Controller][application-gateway-ingress].
...
<details>
  ...
  1. Enter the following values:
    ...
    - **Storage account name**: Enter a new storage account name (e.g., `towerstorage`).
  ...
  1. Select **Review + create**. Note that the default values are used in the other tabs. See [Create a storage account][azure-storage] for further details on each setting.
...
- **Accessible by SSH**.
- Implement DNS or load balancing via [Azure Application Gateway][azure-dns].
...
<details>
  ...
  1. Under **Instance details**:
      - Enter a **VM name**
      - Select the same **Resgion** as your resource group.
  ...
  1. Under **Inbound port rules**:
      ...
      - Select **SSH (22)** and **HTTP (80)** from the dropdown
...
container registry ([cr.seqera.io](https://cr.seqera.io)). Contact [support](https://support.seqera.io) to get your container access credentials. Once you've received your credentials, retrieve the Seqera container images on your Azure VM:
