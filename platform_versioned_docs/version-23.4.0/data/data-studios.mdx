...
If your data studio is very slow to respond it may be because the
AWS Batch compute environment where it's running is being utilized for
other jobs such as running Nextflow pipelines. Data studio sessions
will compete for resources with the Nextflow pipeline head job and
the platform does not currently have an established pattern of
precedence - the compute environment is responsible for scheduling
jobs to the available compute resources available.
...
compare with the compute environment **Compute resources** settings.

### What happens if the memory allocation of the data studio is exceeded?

The running container in the AWS Batch compute environment is given
...
just as if it was running natively on Linux. Linux can overcommit
memory, meaning there is a possibility of getting out-of-memory
reaped in a container environment. The kernel does have protections
in place to prevent this from happening, but it can happen. In that
case, the process gets hard-killed. This can manifest as a performance
lag, killed subprocesses, or a killed data studio session (worst
case scenario).
