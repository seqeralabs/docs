...
Nextflow is a workflow framework that can be used in a wide range of applications to integrate different shell commands into one cohesive pipeline that’s portable, reproducible, scalable, and checkpointed. 

Unless otherwise indicated, processes are run locally. This works well for pipeline development, testing, and small-scale workflows, but for large-scale computational pipelines, a High-Performance Computing (HPC) cluster or cloud platform is recommended.
