...
We officially support the two latest major releases (23.4.x, 23.3.x, etc.) at any given time.
...
:::note 
Nextflow edge versions later than 24.04.2 use Fusion 2.4 by default. 
:::

(* - Nextflow version 24.04.2 will be the baseline version of an upcoming Platform release, but you can take advantage of [new Nextflow features](https://nextflow.io/blog/2024/nextflow-2404-highlights.html) in current Platform versions by specifying `NXF_VER=24.04.2` in your configuration.)

---

To use a Nextflow version other than the baseline in your pipeline run, include the environment variable `NXF_VER` in a [pre-run script](./launch/advanced#pre-and-post-run-scripts) during launch. For example, `NXF_VER=24.04.2`. 
