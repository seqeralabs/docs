...
Since the default for Spot retries is now zero, you must actively enable a retry strategy if you want Nextflow to handle reclaimed Spot instances automatically. For more information, see the [SpotiInstance failures and retries](https://www.nextflow.io/docs/latest/updating-spot-retries.html) guide. 
