...
When you stop and start a Studio session, or start a new one from a previously created checkpoint, changes such as installed software packages and configuration files are restored and made available in the Studio session. Changes made to mounted data are not included in a checkpoint.
