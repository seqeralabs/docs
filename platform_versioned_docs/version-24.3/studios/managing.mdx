...
:::note
The privacy setting for Studios is not linked to the workspace settings. This means that even if you have a shared workspace, you can create a private Studio that only you can view. If a Studio is not **private** all users except those with **View** role can view it. You can change the privacy setting at any time provided the Studio is in a `stopped` state.
:::
...
You can start a new session from an existing stopped session. This will inherit the history of the parent checkpoint state. From the list of **stopped** Studios in your workspace, select the three dots next to the status message for the Studio you want to start and select **Start as new**. Alternatively, select the **Checkpoints** tab on the detail page, select the three dots in the **Actions** column, and then select **Start as new** to start a new session.
