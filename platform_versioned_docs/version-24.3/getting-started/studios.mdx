...
In this section, we'll demonstrate how to set up a Jupyter notebook Studio for visualizing results from [nf-core/proteinfold](https://nf-co.re/proteinfold/1.1.1), a bioinformatics best-practice analysis pipeline for Protein 3D structure prediction.
...
The following instructions set up a Jupyter notebook to visualize and compare protein structures produced by Alphafold 2 and ESMFold, creating an interactive visualization of the two structures overlayed in one image with contrasting colors. The script retrieves the CA atoms from both structures, checks whether the structures have the same number of CA atoms, creates a contact map, calculates the per-residue RMSD for each structure, and visualizes the structures, secondary structures, and per-residue RMSD using PyMOL. 

The Jupyter environment can be configured with the packages and scripts you need for interactive analysis. Run the following scripts in individual code cells to install the necessary packages and perform visualization:
...
1. Search for PCSK9 and zoom into one of the exons of the gene. If you are on genome version hg19 and everything worked as expected, you should see a coverage graph and reads as shown below:
    ...
    ![BAM file view](xpra-data-studios-IGV-view-bam.png)
