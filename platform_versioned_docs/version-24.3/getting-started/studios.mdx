...
Studios require an AWS Batch compute environment. If you do not have an existing compute environment available, create one with the following attributes:
...
Studios require an AWS Batch compute environment. If you do not have an existing compute environment with at least 2 CPUs available, create one with the following attributes:
...
![Add a Studio](./_images/create-ds.gif)
...
- In the **Compute & Data** tab:
    - Select your AWS Batch compute environment. 
      ...
      :::    
    - Allocate at least 4 CPUs and 16000 MB RAM. 
