...
:::note
If you specify a custom script filename, the root of the default branch in your pipeline repository must still contain blank `main.nf` and `nextflow.config` files. See [Nextflow configuration](../ftroubleshooting_and_faqs/nextflow.mdx) for more information on this known Nextflow behavior.
:::
