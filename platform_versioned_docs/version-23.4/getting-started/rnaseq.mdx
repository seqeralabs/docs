...
![nf-core/rnaseq subway map](./_images/nf-core-rnaseq_metro_map_animated.svg)
:::
...
The [nf-core/rnaseq](https://github.com/nf-core/rnaseq) pipeline works with input datasets (samplesheets) containing sample names, FASTQ file locations, and indications of strandedness. For example, the dataset used in the `test_full` profile is derived from the publicly available iGenomes collection of datasets, commonly used in bioinformatics analyses. 

This dataset represents RNA-seq samples from various human cell lines (GM12878, K562, MCF7, and H1) with biological replicates, stored in AWS S3 buckets as part of the iGenomes resource. These RNA-seq datasets consist of paired-end sequencing reads, which can be used to study gene expression patterns in different cell types.
...
<details>
  ...
  The run details page includes general information about who executed the run and when, the Git commit ID and/or tag used, and additional details about the compute environment and Nextflow version used.
