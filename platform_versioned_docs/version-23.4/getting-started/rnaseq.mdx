...
The RStudio environment can be configured with the packages you wish to install and the R scripts you wish to run. For the purposes of this guide, the following script will be used to install the necessary packages and perform the analysis:
...
2. Reads in the count data and sample information.
3. Creates a DGEList object and filters out low-count genes.
4. Normalizes the data.
1. Displays a summary of the filtered data. 
6. Creates an MDS plot (both displayed in RStudio and saved as a PNG file).
7. Performs differential expression analysis for each specified contrast.
8. Prints the number of differentially expressed genes for each comparison and saves the full results to CSV files.
9. Creates volcano plots for each comparison (both displayed in RStudio and saved as PNG files).
10. Creates a heatmap of the top differentially expressed genes (both displayed in RStudio and saved as a PNG file).
