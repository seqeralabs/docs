...
- Creating an AWS Batch compute environment to run your pipeline and analysis data studio
...
- Setting up a tertiary analysis environment with Data Studios
...
- Existing access credentials with permissions to create and manage resources in your AWS account.
:::
...
The following compute resources are recommended for production pipelines, depending on the size of your input dataset:

| **Setting**                    | **Value**                             |
|--------------------------------|---------------------------------------|
| **Instance Types**             | `m5.large` - `m5.2xlarge`             |
| **vCPUs**                      | 2 - 8                                 |
| **Memory (GiB)**               | 8 - 32                                |
| **Max CPUs**                   | >500                                  |
| **Min CPUs**                   | 0                                     |
...
The [Fusion](../supported_software/fusion/fusion.mdx) file system enables seamless filesystem I/O to cloud object stores via a standard POSIX interface, resulting in
simpler pipeline logic and faster, more efficient pipeline execution. While Fusion is not required for pipeline execution, it is recommended for optimal performance. See [Compute benchmarks](#compute-benchmarks) at the end of this guide.

We recommend using Fusion with AWS NVMe instances (fast instance storage) as this delivers the fastest performance when compared to environments using only AWS EBS (Elastic Block Store). Batch Forge selects instances automatically based on your compute environment configuration, but you can optionally specify instance types. To enable fast instance storage (see Create compute environment below), you must select EC2 instances with NVMe SSD storage (`m5d.large` - `m5d.2xlarge` or greater). 
...
| **Allowed S3 buckets**                | Additional S3 buckets or paths to be granted read-write permission for this compute environment. Add data to be mounted in your data studio here.|
...
- Use [resource labels](../resource-labels/overview.mdx) to tag the computing resources created during the workflow execution. While resource labels for the run are inherited from the compute environment and pipeline, admins can override them from the launch form. Applied resource label names must be unique. 
- [Pipeline secrets](../secrets/overview.mdx) are used to store keys and tokens used by workflow tasks to interact with external systems. Enter the names of any stored user or workspace secrets required for the workflow execution.
...
For the purposes of this guide, we will create an RStudio environment in which to normalize the pipeline output data, perform differential expression analysis, and visualize the data with MDS plots, volcano plots, and a heatmap of the most differentially expressed genes.  

### Prepare your data

#### Gene counts 

Salmon is the default tool used during the `pseudo-aligner` step of the nf-core/rnaseq pipeline. In the pipeline output data, the `/salmon` directory contains the tool's output, including a `salmon.merged.gene_counts.tsv` file. 

#### Sample info

The analysis script provided in this section requires a sample information file to parse the counts data in the `salmon.merged.gene_counts.tsv` file. nf-core/rnaseq does not produce this sample information file automatically. To create a sample information file based on the genes in your `salmon.merged.gene_counts.tsv` file:

1. Note the names of the columns (excluding the first column, which typically contains gene IDs) in your `salmon.merged.gene_counts.tsv` file. These are your sample names.
...
1. For each sample in your `salmon.merged.gene_counts.tsv` file:
...
The RStudio environment can be configured with the packages you wish to install and the R script you wish to run. For the purposes of this guide, the following script will be used to install the necessary packages and perform the analysis:
...
# 1b. Load required libraries
library(limma)
library(edgeR)
library(ggplot2)
library(gplots)

# 2a. Read in the count data
counts <- read.delim(file="/workspace/data/<PATH_TO_YOUR_COUNTS_FILE>", row.names=1)

# 2b. Remove the gene_name column if it exists
if("gene_name" %in% colnames(counts)) {
  counts <- counts[, -which(colnames(counts) == "gene_name")]
}

# 2c. Convert to matrix
counts <- as.matrix(counts)

# 2d. Read in the sample information
targets <- read.table(file="/workspace/data/<PATH_TO_YOUR_SAMPLE_INFO_FILE>", 
                      header=TRUE, stringsAsFactors=FALSE, sep="", check.names=FALSE)

# 2e. Ensure column names are correct
colnames(targets) <- c("Sample", "Group")

# 3a. Create a DGEList object
y <- DGEList(counts, group=targets$Group)

# 3b. Calculate CPM (counts per million) values
mycpm <- cpm(y)

# 3c. Filter low count genes
thresh <- mycpm > 0.5
...
# 5. Print summary of filtered data
print(dim(y))
print(y$samples)

# 6. Create MDS plot
# 6a. Display in RStudio
...
# 7e. Perform differential expression analysis for each contrast
...
# 8a. Print the number of differentially expressed genes for each comparison
...
# 9. Create volcano plots for differential expression comparisons
# 9a. Function to create a volcano plot
create_volcano_plot <- function(res, title) {
  ggplot(res$table, aes(x = logFC, y = -log10(FDR))) +
    geom_point(aes(color = FDR < 0.05 & abs(logFC) > 1), size = 0.5) +
    scale_color_manual(values = c("black", "red")) +
    labs(title = title, x = "Log2 Fold Change", y = "-Log10 FDR") +
    theme_minimal()
}

# 9b. Create volcano plots for each comparison
for (name in names(results)) {
  p <- create_volcano_plot(results[[name]], name)
  # Display in RStudio
  print(p)
  # Save to file
  ggsave(paste0("volcano_plot_", name, ".png"), p, width = 8, height = 6, dpi = 300)
}

# 10. Create a heatmap of top 50 differentially expressed genes
# 10a. Get top 50 DE genes from each comparison
top_genes <- unique(unlist(lapply(results, function(x) rownames(x$table)[1:50])))

# 10b. Get log-CPM values for these genes
log_cpm <- cpm(y, log = TRUE)
top_gene_expr <- log_cpm[top_genes,]

# 10c. Print dimensions of top_gene_expr
print(dim(top_gene_expr))

# 10d. Create a color palette
my_palette <- colorRampPalette(c("blue", "white", "red"))(100)

# 10e. Create a heatmap using heatmap.2
# Display in RStudio
heatmap.2(as.matrix(top_gene_expr), scale = "row", col = my_palette,
...
png("heatmap_top_DE_genes.png", width = 1000, height = 1200)
heatmap.2(as.matrix(top_gene_expr), scale = "row", col = my_palette,
          trace = "none", dendrogram = "column", margins = c(5, 10),
          labRow = FALSE, 
          ColSideColors = rainbow(length(unique(y$samples$group)))[factor(y$samples$group)],
...
# 10g. Print the number of top genes in the heatmap
print(paste("Number of top DE genes in heatmap:", length(top_genes)))
```

This script is written for RNA-seq and differential expression analysis of human data, based on nf-core/rnaseq's `test_full` dataset. The script performs the following steps:

1. Installs and loads necessary libraries.
1. Reads in the count data and sample information.
1. Creates a DGEList object and filters out low-count genes.
1. Normalizes the data.
1. Displays a summary of the filtered data. 
1. Creates an MDS plot (both displayed in RStudio and saved as a PNG file).
1. Performs differential expression analysis for each specified contrast.
1. Prints the number of differentially expressed genes for each comparison and saves the full results to CSV files.
1. Creates volcano plots for each comparison (both displayed in RStudio and saved as PNG files).
1. Creates a heatmap of the top differentially expressed genes (both displayed in RStudio and saved as a PNG file).

#### Customize the R script 

1. In steps `2a` and `2d` of the R script in the previous section, specify the paths to your `salmon.merged.gene_counts.tsv` and `sampleinfo.txt` files.
1. In step `7d`, modify the contrasts based on the comparisons you want to make between your sample groups:
...
1. Each of the plots produced by the script (MDS plots, volcano plots, and a heatmap) are displayed in the RStudio plots viewer and saved to a PNG file. You can remove the `save to file` section for each plot, or change the output file format as needed. 
...
The compute recommendations in this guide are based on internal benchmarking performed by Seqera. Benchmark runs of [nf-core/rnaseq](https://github.com/nf-core/rnaseq) used profile `test_full`, consisting of an input dataset with 16 FASTQ files and a total size of approximately 123.5 GB.

This benchmark compares pipeline run metrics between an AWS Batch compute environment with Fusion file system and fast instance storage enabled (**Fusionv2** group) and an identical AWS Batch compute environment using S3 storage without Fusion (**plainS3** group).

#### Overall run metrics

The Fusion file system used with NVMe instance storage contributed to a 34 % improvement in total pipeline runtime and a 49 % reduction in CPU hours, with a 7 % improvement in CPU efficiency.
