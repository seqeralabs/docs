...
Seqera requires SSH access to your HPC cluster to run pipelines. Use [managed identities](../credentials/managed_identities.mdx) to enable granular access control and preserve individual cluster user identities. If you do not need to grant multiple users access to your cluster, use workspace [SSH credentials](../credentials/ssh_credentials.mdx) for cluster login. 
...
4.  Select your managed identity, Tower Agent, or SSH credentials, or select **+** and **SSH** or **Tower Agent** to add new credentials.
