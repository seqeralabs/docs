...
12. If the selected Azure Batch Pool has an attached user-assigned **Managed Identity**, you can choose it here.
...
15. Click **Add** to complete the compute environment setup. The creation of resources will take a few seconds, after which you can launch pipelines.

### Managed Identity

Nextflow can authenticate to Azure services using a Managed Identity, offering enhanced security compared to access keys, but this method requires running on Azure infrastructure. When using the Seqera Platform with a manually configured compute environment that has a Managed Identity attached to the Azure Batch Pool, Nextflow can utilize this Managed Identity for authentication. However, the Seqera Platform still needs to use access keys to submit the initial task to Azure Batch to run Nextflow, which will then proceed with the Managed Identity for subsequent authentication.

1. In Azure, create a user-assigned Managed Identity. Refer to the [Azure documentation](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/how-manage-user-assigned-managed-identities) for detailed steps. After creation, make sure to record the Client ID of the Managed Identity.
2. Ensure the user-assigned Managed Identity has the necessary roles for Nextflow. Consult the [Nextflow documentation](https://www.nextflow.io/docs/latest/azure.html#required-role-assignments) for role requirements.
3. Associate the user-assigned Managed Identity with the Azure Batch Pool. Detailed instructions can be found in the [Azure documentation](https://learn.microsoft.com/en-us/troubleshoot/azure/hpc/batch/use-managed-identities-azure-batch-account-pool#set-up-managed-identity-in-your-batch-pool).
4. When setting up the Seqera Platform Compute Environment, select the Azure Batch Pool by name and enter the Managed Identity Client ID in the specified input field as instructed above.

After submitting a pipeline to this compute environment, Nextflow will authenticate using the Managed Identity associated with the Azure Batch node it is running on, rather than relying on access keys.
