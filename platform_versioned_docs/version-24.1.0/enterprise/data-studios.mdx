...
Enable Data Studios as part of your Seqera Platform instance. Only the Amazon AWS public cloud is currently supported.

:::caution 
You must complete the upgrade of your Platform Enterprise installation to version 24.1.0 before enabling and configuring Data Studios.
:::
...
1. Edit the `proxy.yml` file and set the following variables:
    ...
    - `CONNECT_PROXY_URL`: A base domain URL for the connect proxy subdomain. We recommend you set the base URL for your Platform installation (`PLATFORM_URL` below) for your connect proxy, to be able to use the same wildcard TLS certificate for all data studio instance URLs and avoid additional domain nesting. 
...
1. Edit your Platform installation's `ingress.eks.yml` file:
    ...
    - Replace `<CONNECT_DOMAIN>` with the domain of your `CONNECT_PROXY_URL` value specified in the `proxy.yml` file above. For example, if your connect proxy URL is `https://example.com`, set the `<CONNECT_DOMAIN>` value as `example.com`.
