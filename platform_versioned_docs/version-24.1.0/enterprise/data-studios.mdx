...
1. Download the Kubernetes manifest for the Data Studios service.

1. Create a Secret that contains the initial access token:
...
1. Edit your ConfigMap named `platform-backend-cfg` for Platform and add the following environment variables:
...
1. Apply your updated configuration: `kubectl apply -f configmap.yml`
