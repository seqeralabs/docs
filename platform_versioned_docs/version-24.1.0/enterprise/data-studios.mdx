...
1. Open `data-studios.env` in an editor, and make the following changes:
   ...
   1. Set the following environment variables:
      ...
      - `CONNECT_PROXY_URL`: Specify a base domain name for the connect proxy subdomain. For example, if you specify the base domain `example.com` then `connect.example.com` is the fully qualified domain name for the proxy.
      - `CONNECT_OIDC_CLIENT_REGISTRATION_TOKEN`: Specify the same base64-encoded value set in the `client_token` environment variable.
...
1. Open `data-studios.env` in an editor and set the following variables:
    ...
    - To restrict Data Studios to specific organization workspaces, provide a comma-separated list of workspace IDs in the `TOWER_DATA_STUDIO_ALLOWED_WORKSPACES` environment variable.
    - To allow access to Data Studios in every organizational workspace, do not set the `TOWER_DATA_STUDIO_ALLOWED_WORKSPACES` environment variable.
...
1. Edit the ConfigMap named `platform-backend-cfg` in the `configmap.yml` for Platform by adding the following environment variables:
    ...
    - To restrict Data Studios to specific organization workspaces, provide a comma-separated list of workspace IDs in the `TOWER_DATA_STUDIO_ALLOWED_WORKSPACES` environment variable.
    - To allow access to Data Studios in every organizational workspace, do not set the `TOWER_DATA_STUDIO_ALLOWED_WORKSPACES`.

1. Optional: To allow access to Data Studios in every organizational workspace, edit the ConfigMap named `tower-yml` in the `configmap.yml` and include the following snippet:
