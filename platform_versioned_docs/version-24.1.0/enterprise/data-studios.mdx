...
1. Edit the `proxy.yml` file and set the following variables:
    ...
    - `REDIS_SERVER`: Specify the host name or IP address of the Redis server that you configure for Platform.
    - `TOWER_CONNECT_DOMAIN`: Specify a base domain name for the connect proxy subdomain. For example, if you specify `example.com` then `connect.example.com` is the fully qualified domain name for the proxy.
    - `TOWER_BASE_URL`: Specifies the base URL for your Platform installation, such as `https://tower.example.com/`.
