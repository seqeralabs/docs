...
`tower.io`, then proxy domain should be `example.tower.io`).
...
!!! warning It is recommended to use only a third level domain if possible, as the cost may increase with the number of subdomains.