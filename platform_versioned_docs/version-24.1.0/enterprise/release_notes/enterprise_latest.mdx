...
- A redesigned [pipeline launch form](../../launch/launchpad.mdx#launch-form) provides an improved workflow and the option to provide run parameters as raw JSON/YAML. Enable the new launch form per workspace via [environment variable](../enterprise/configuration/overview.mdx#core-features).
