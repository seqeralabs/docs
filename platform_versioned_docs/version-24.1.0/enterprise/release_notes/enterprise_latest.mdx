...
- The standard email login can be disabled in the `tower.yml` file in the `TOWER_LOGIN_DISABLE_EMAIL=true` environment variable provided an alternative OIDC provider is set up first. See, [the configuration overview page](./configuration/overview.mdx) for more information.
