...
[Data Explorer](./data-explorer.mdx) is now Generally Available and supports multi-file and multi-folder download. This allows users download entire datasets of pipeline results to share with their team. This is an alternative to traditional cloud storage services without requiring a command-line interface or user interfaces.

## Extended data roles **< add link to content when platform PR is merged >**

The **Connect** user role has been added to the existing user roles. User roles provide flexibility for admins to provide users with the permissions they need, without compromising security. Existing roles have been updated with functionality related to Data Studios and Data Explorer:
...
Seqera managed identities resolve the limitations of SSH access by enabling granular access control to HPC clusters for individual workspace users.

## Other feature improvements

- Retrieve reports from primary compute environment on Nextflow CLI runs. **< include link to content when merged >**
- The standard email login can be disabled in the `tower.yml` file in the `TOWER_LOGIN_DISABLE_EMAIL=true` environment variable provided an alternative OIDC provider is set up first. See, [the configuration overview page](https://docs.seqera.io/platform/24.1.0/enterprise/configuration/overview) for more information.

## Breaking changes

**MariaDB driver: New MySQL connection parameter required**

MariaDB driver 3.x requires a special parameter in the connection URL to connect to a MySQL database (`?permitMysqlScheme=true`):

`jdbc:mysql://<domain>:<port>/tower?permitMysqlScheme=true`

All deployments using a MySQL database (regardless of version: 5.6, 5.7, or 8) should be updated accordingly once a Platform version including the upgrade is deployed.

**MariaDB driver: No truncation support for MySQL 5.6**

The MariaDB driver has dropped support for the `jdbcCompliantTruncation` param, which was true by default and always set the `STRICT_TRANS_TABLES` SQL mode (the `STRICT_TRANS_TABLES` mode throws an error when the value of a VARCHAR column exceeds its limit, instead of truncating it to fit). Most common installations of MySQL 5.7 and 8 already include this mode at the server level, but the Docker container version of MySQL 5.6 does not.
...
:::caution
If you are upgrading from a version older than 23.4.1, update your installation to version 23.4.4 first, before updating to 24.1.0 using the steps below.
:::
