...
The Administration Console allows Tower instance administrators to interact with all users and organizations registered with the platform. Administrators must be identified in your Seqera instance configuration files prior to instantiation of the application.
...
**<p data-question>Q: I'm trying to query more results than the maximum return size allows. Can I do pagination?</p>**
...
Starting with v22.1, Seqera Platform implements stricter cookie security by default and will only send an auth cookie if the client is connected via HTTPS. The lack of an auth token will cause HTTP-only log-in attempts to fail (thereby causing the frozen screen).
...
A known issue with versions prior to 22.3 caused resuming runs to fail for users with the launch role. This issue was fixed in version 22.3. Upgrade to the latest version of Seqera to allow launch users to resume runs.

## Compute Environments

**<p data-question>Q: Can the name of a Compute Environment created in Seqera contain special characters?</p>**

No. version 21.12 and later do not support the inclusion of special characters in the name of Compute Environment objects.
