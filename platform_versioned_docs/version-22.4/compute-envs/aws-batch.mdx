...
- You can control the error strategy settings (`errorStrategy`, `maxRetries`) for Batch jobs submitted by Nextflow in **Staging options > Nextflow config**. This ensures you don't incur unneccessary costs. For example:
