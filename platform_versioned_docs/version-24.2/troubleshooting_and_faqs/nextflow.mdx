...
From v24.10, the default setting was changed to `0` for Spot retries on AWS and Google, meaning that no ‘internal’ retries are attempted on these platforms by default. Spot reclamations now lead to an immediate failure, exposed to Nextflow in the same way as other generic failures (returning for example, `exit code 1` on AWS). Nextflow will treat these failures like any other job failure unless you actively configure a retry strategy.
...
Since the default for spot retries is now zero, you must actively enable a retry strategy if you want Nextflow to handle reclaimed Spot instances automatically. See [the changelog](https://docs.seqera.io/changelog/tags/seqera-cloud#breaking-changes) for more information.
