...
1. Expand **Staging options** to include:
    ...
    - Global Nextflow configuration settings for all pipeline runs launched with this compute environment. Values defined here are pre-filled in the **Nextflow config file** field in the pipeline launch form. These values can be overridden during pipeline launch. 
    - You can specify the the [error strategy](https://www.nextflow.io/docs/latest/reference/process.html#errorstrategy) settings (`errorStrategy`, `maxRetries`) for Batch jobs submitted by Nextflow in **Staging options > Nextflow config**. This ensures you don't incur unnecessary costs. For example:
