...
- Expand **Staging options** to include:
    ...
    - You can change the the [error strategy](https://www.nextflow.io/docs/latest/reference/process.html#errorstrategy) settings (`errorStrategy`, `maxRetries`) for Batch jobs submitted by Nextflow in **Staging options > Nextflow config**. This ensures you don't incur unneccessary costs. For example:
