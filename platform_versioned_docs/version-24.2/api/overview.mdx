...
<p>
  <Button label="View API reference" link="https://cloud.seqera.io/openapi/index.html" wrapParagraphClassName="float-right" className="mr-3" />{" "}
  <Button label="Download API schema" link="https://cloud.seqera.io/openapi/seqera-api-latest.yml" outline />
</p>

The API reference is an interactive list of all API endpoints and includes request and response payload examples to test requests in the browser.

:::info
The API requires authentication with a Platform access token to perform requests.

<Button label="Create an access token" link="https://cloud.seqera.io/tokens" outline />
:::
...
Both the [Seqera Platform CLI](../cli/overview.mdx) and [seqerakit](https://github.com/seqeralabs/seqera-kit) are built on the Platform API and provide a programmatic automation layer for operations.
