...
When a change to session lifespan is made, it applies only to newly started sessions. Running sessions are not effected by changes to the session lifespan.
