...
You can set a lifespan for a Studio session. Upon expiration, a snapshot is taken and then the session is automatically shutdown. A session can be extended multiple times before it is shut down. When a change to session lifespan is made, it applies only to newly-started sessions. Running sessions are not affected by changes to the session lifespan.
