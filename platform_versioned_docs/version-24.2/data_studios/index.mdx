...
The image template tag includes the version of the analysis application and the Seqera image template version itself. The tag string looks like this:

```ignore title="Image template tag"
<tool_version>-[u<update_version>]-<seqera_version>
```

- `<tool_version>`: Analysis application version, such as `4.2.5` for JupyterLab
- `<update_version>`: Seqera image template update integer version, such as `u1`, for instances where a breaking change is introduced
- `<seqera_version>`: Seqera template image version, such as `0.7.0`

Additionally, the Seqera version string looks like this:

```ignore title="Seqera version tag subset"
<major>.<minor>.<patch>
```

- `<major>`: Signifies breaking changes in the underlying Seqera container
- `<minor>`: Signifies non-breaking changes in the underlying Seqera container
- `<patch>`: Signifies minor non-breaking changes in the underlying Seqera container

When pushed to the container registry, an image template is tagged with the following tags:

- `<major>.<minor>`, such as `4.2.3-0.7`. When adding a new data studio this tag is the tag displayed in Seqera Platform.
- `<major>.<minor>.<patch>`, such as `4.2.3-0.7.1`.

To see the latest versions of the images, see [public.cr.seqera.io](https://public.cr.seqera.io/). You can also create your own container image templates. For more information, see [Custom environments][custom-envs].
