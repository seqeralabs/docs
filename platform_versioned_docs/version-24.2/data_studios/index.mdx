...
- **running**: When a Studio is **running**, you can connect to it, copy the URL, or stop it. In addition, it can continue to process requests/run computations in the absence of an ongoing connection.
- **stopping**: The recently-running Studio is in the process of being stopped.
...
- **errored**: This state most often indicates that there has been an error starting the Studio but it is in a **stopped** state. There might be errors reported by the Studio itself but these will be overwritten with a **running** status if the session is still running.
...
By default a Studio is collaborative. All workspace members can connect to a running session with the exception of those with only the **View** role. If a session is configured to run in private mode, only the user who created the Studio can connect to it.

## Session management

You can set a session lifespan for a Studio. Upon expiration, a snapshot is taken and then the session is automatically shutdown. A session can be extended multiple times before it is shut down. When a change to session lifespan is made, it applies only to newly-started sessions. Running sessions are not effected by changes to the session lifespan.
...
When you stop and start a Studio, or start a new Studio from a previously created checkpoint, changes such as installed software packages and configuration files are restored and made available in the Studio. Changes made to mounted data are not included in a checkpoint.
...
Checkpoint files in the compute environment work directory may be shared by multiple Studios. Each checkpoint file is cleaned up asynchronously after the Studio that referenced the checkpoint is deleted.
...
Stop the active Studio to trigger a snapshot from the active volume. The snapshot is uploaded to cloud storage with Fusion. When you start from the newly-saved snapshot, all previous data is loaded and the newly-started Studio will have 2 GB of available space.
