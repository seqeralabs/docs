...
```ignore title="Seqera version"
<major>-<minor>-<patch>-[u-<update_version>]
```
...
- `<update_version>`: Seqera image template update version for instances where a breaking change is introduced

When pushed to the container registry, an image template is tagged with a `<major>.<minor>` tag. For example, the `data-studio-jupyter` image template with the tag `:4.2.3-0.7.1` is also tagged as `data-studio-jupyter:4.2.3-0.7`. Use of only the `<major>.<minor>` is preferred. Seqera recommends that you use the latest available version of the container image templates.

To see the latest versions of the images, see [public.cr.seqera.io](https://public.cr.seqera.io/).
