...
You can configure a Data Studio to include one or more data links, where cloud buckets that you have configured in your compute environment are exposed within the studio session.
...
You can limit write access to just a subdirectory of a data bucket by creating an additional data bucket for only that subdirectory, and then using this bucket as a data link. For example, if you have the following S3 buckets:

- `s3://biopharmaXs`: Entire bucket
- `3://biopharmaX/experiments/project-A/experiment-1/data`: Subdirectory for use from a studio session

You can expose the `s3://biopharmaX/experiments/project-A/experiment-1/data` bucket subdirectory in a studio session as `/workspace/data/biopharmaxs-project-a-experiment-1`.

For more information, see [Limit Data Studio access to a specific cloud bucket subdirectory][cloud-bucket-subdirectory].
