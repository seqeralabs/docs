...
:::caution
The Seqera container registry `cr.seqera.io` is the default Seqera Enterprise container image registry from version 22.4. AWS, Azure, and Google Cloud Seqera image registries in existing installations are still supported but are deprecated for **new installations** from June 2023.
:::

???+ example "Legacy Seqera image instructions"
    ...
    === "Azure"
        Seqera publishes legacy Seqera Enterprise containers to a private Azure Container Registry instance. Retrieve them with the following steps:

        1. **Acquire credentials from Seqera.**

            If you chose to retrieve your Seqera Enterprise containers from Seqera's Azure Container Registry, you were supplied with a user ID and authentication token during the onboarding process.
    ...
    === "Google Cloud"
        Seqera publishes legacy Seqera Enterprise containers to a private Artifact Registry (AR) on GCP. Retrieve them with the following steps:
