...
Run the Seqera container with [Docker](../docker-compose.mdx) on a GCP VM instance or with [Kubernetes](../kubernetes.mdx) on a Google GKE cluster. You must satisfy the requirements for your installation target:
