...
<TabItem value="AWS Batch with Fusion v2" label="AWS Batch with Fusion v2" default>
  ...
  1. Scroll down to **Advanced details** and paste the following in the **User data** field:
    ...
    write_files:
      - path: /root/seqera-ce.sh
        ...
        content: |
          ...
          #Uncomment either the AMD64 or ARM64 line below to install the correct version of Amazon CloudWatch Agent for your CPU architecture
          #wget -q https://amazoncloudwatch-agent.s3.amazonaws.com/amazon_linux/amd64/latest/amazon-cloudwatch-agent.rpm
          #wget -q https://amazoncloudwatch-agent.s3.amazonaws.com/amazon_linux/arm64/latest/amazon-cloudwatch-agent.rpm
          rpm -U ./amazon-cloudwatch-agent.rpm
          rm -f ./amazon-cloudwatch-agent.rpm
          curl -s https://github.com/seqeralabs/nf-tower-aws/tree/master/cloudwatch-config/agent-config.json \
            | sed 's/seqera-prefix/<your prefix>/g' \
    ...
    runcmd:
      - bash /root/seqera-ce.sh

    --//--
    ```

  1. Uncomment the appropriate `wget` line to download the version of Amazon Cloudwatch Agent for your CPU architecture (AMD64 or ARM64).
  1. In the line `| sed 's/seqera-prefix/<your prefix>/g' \`, replace `<your prefix>` with your desired prefix to be added to the AWS resources created by your compute environment.
...
<TabItem value="AWS Batch without Fusion v2" label="AWS Batch without Fusion v2" default>
    ...
    write_files:
      - path: /root/seqera-ce.sh
        ...
        content: |
          ...
          rpm -U ./amazon-cloudwatch-agent.rpm
          rm -f ./amazon-cloudwatch-agent.rpm
          curl -s https://docs.seqera.io/assets/files/agent-config-d9b0cff85ec4cc20e44262db76ebc5e4.json \
            > /opt/aws/amazon-cloudwatch-agent/bin/config.json
          /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl \
            -a fetch-config \
            -m ec2 \
  ...
  1. Uncomment the appropriate `wget` line to download the version of Amazon Cloudwatch Agent for your CPU architecture (either AMD64 or ARM64). Download the [cloudwatch-agent](../_templates/cloudwatch/agent-config.json) to customize it. 
  1. In the line `| sed 's/seqera-prefix/<your prefix>/g' \`, replace `<your prefix>` with your desired prefix to be added to the AWS resources created by your compute environment.
  1. Save the template with the name `seqera-launchtemplate`.
