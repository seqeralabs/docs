...
<details>
  ...
  In the **Data Explorer** tab, search for `seqeralabs-showcase/samplesheet-location/samplesheet.csv` and select the `samplesheet.csv` file.
...
<details>
  ...
  In the **Datasets** tab, search for and select a pre-loaded dataset called `rnaseq_sample_data`.
...
:::tip
- See [Add a cloud bucket](./data-explorer.mdx#add-a-cloud-bucket) to add private and public cloud storage buckets to your workspace.
- See [Add a dataset](./add-datasets.mdx#add-a-dataset) to upload your own samplesheets in the **Datasets** tab of your workspace.
:::
