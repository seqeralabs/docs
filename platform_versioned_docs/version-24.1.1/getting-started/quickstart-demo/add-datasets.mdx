...
- Select **Upload file** and browse to your downloaded `samplesheet_test.csv` file. This CSV file specifies the paths to 7 small FASTQ files for a sub-sampled yeast RNAseq dataset.

Notice the location of the files point to a path on S3. This could also be a path to a shared filesystem, if using an HPC compute environment. Nextflow will use these paths to stage the files into the task working directory. 
