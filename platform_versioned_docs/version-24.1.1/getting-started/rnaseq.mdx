...
This guide details how to run RNA sequencing data analysis, from quality control to differential expression analysis, on a cloud compute environment in Platform, including: 
...
:::info
**Prerequisites**
This guide includes setting up a cloud compute environment in Platform. You will need the following to get started:
- [Admin](../orgs-and-teams/roles.mdx) permissions in an existing organization workspace.
...
:::info
All nf-core pipelines include a `test_full` profile that is associated with a test dataset. The following specifications are a guideline based on internal runs of nf-core/rnaseq performed with the `test_full` configuration profile, which uses a dataset with **x** samples and a total size of **x** GB.
:::
...
:::info
To add a custom pipeline not listed in Seqera Pipelines to your Platform workspace, see [this example](./quickstart-demo/add-pipelines.mdx#) for manual Launchpad instructions. 
:::
...
With your compute environment created, the nf-core/rnaseq pipeline added to your workspace Launchpad, and your samplesheet accessible in Platform, you are ready to launch. Navigate to the Launchpad in the `community/showcase` workspace and select **Launch** next to the `nf-core-rnaseq` pipeline to open the launch form.
...
Use **Browse** to select either the cloud storage file path to your samplesheet via **Data Explorer**, or your uploaded **Dataset**: 
