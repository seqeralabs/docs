...
Seqera Platform is available in two [deployment editions](../getting-started/deployment-options.mdx) and can be accessed via web user interface (UI), [API](../api/overview.mdx), [CLI](.overview.mdx), or directly in your Nextflow command using a flag.
