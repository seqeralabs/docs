...
- `TW_AGENT_WORKDIR`: Sets the work directory for the agent. By default, this directory resolves to the `${HOME}/work` path, where `HOME` is the home directory of the user that the agent runs as. The work directory can also be overridden by specifying the `--work-dir` argument.
- `TW_AGENT_USER`: Sets the user name that the agent identifies as running as. By default, this is the Linux user name that the agent runs as and is the value of the `USER` environment variable.
