...
#### AWS Batch Forge compute environments with Fusion only
...
#### Azure Batch Forge compute environments with Fusion
...
1. For this configuration, use Seqera Platform version 23.1 or later.
1. Create an [Azure Batch compute environment](../../compute-envs/azure-batch.mdx#batch-forge).
...
1. Specify suitable VM sizes under **VMs type**. 

:::tip
We recommend selecting machine types with an attached temp storage disk of at least 512Gb and random read of 1000 IOPS for large and long-lived production pipelines. The suffix `d` after the core number (e.g. `Standard_E32*d*_v5`) denotes a VM with a local temp disk. Larger local storage increases Fusion's throughput and reduces the chance of overloading the machine. See [Sizes for virtual machines in Azure](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/overview) for more information.
:::

#### Google Cloud Batch coompute environments with Fusion
