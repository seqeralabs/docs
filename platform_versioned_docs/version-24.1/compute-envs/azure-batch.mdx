...
10. Select **Enable Fusion v2** to allow access to your Azure Blob Storage data via the [Fusion v2][nf-fusion-docs] virtual distributed file system. This speeds up most data operations. The Fusion v2 file system requires Wave containers to be enabled. See [Fusion file system](../supported_software/fusion/fusion.mdx) for configuration details.
    ...
    1. Specify suitable VM sizes under **VMs type**. Seqera benchmark testing results recommend a `Standard_E16d_v5` VM or larger for production use. Benchmarking consisted of X concurrent runs of X nf-core pipelines, with a total input data size of X. 

    :::tip
    We recommend selecting machine types with a local temp storage disk of 512Gb and random read of 1000 IOPS or more for large and long-lived production pipelines. Select instances with Standard SSDs — Fusion does not support Azure network-attached storage (Premium SSDv2, Ultra Disk, etc.). The suffix `d` after the core number (e.g. `Standard_E16*d*_v5`) denotes a VM with a local temp disk. Larger local storage increases Fusion's throughput and reduces the chance of overloading the machine. See [Sizes for virtual machines in Azure](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/overview) for more information.
    :::
