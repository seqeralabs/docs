...
To use [Fusion v2](../supported_software/fusion/fusion.mdx) in your Seqera EKS compute environment:
...
We recommend using Fusion with AWS NVMe instances as this delivers the fastest performance when compared to environments using only AWS EBS (Elastic Block Store). 
