...
- **building**: A custom environment is being built using the [Wave](/wave/) service that will serve as a template image for a new data studio session
- **build-failed**:  A custom environment build failed. This is a non-recoverable error. Logs are provided to assist with troubleshooting.
