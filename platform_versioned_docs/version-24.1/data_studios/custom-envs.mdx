...
## Inspect custom container template build status {#build-status}

You can inspect the progress of a custom container template build, including any errors if the build fails. A link to the [Wave service][wave-home] container build report is always available for builds.
...
1. From the list of data studio sessions, select the name of the session with **building** or **build-failed** status that you want to inspect.
