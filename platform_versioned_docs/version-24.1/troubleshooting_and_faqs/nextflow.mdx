...
Currently, you can resolve this by creating empty `main.nf` and `nextflow.config` files in the default branch. This allows the pipeline to run,by using the content of the `main.nf` and `nextflow.config` in your target revision.
