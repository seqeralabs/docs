...
Data Studios is a unified platform where you can host a combination of container images and compute environments for interactive tertiary analysis using your preferred tools, like JupyterLab and RStudio Notebooks, Visual Studio Code IDEs, or Xpra remote desktops. Each data studio session is an individual interactive environment that encapsulates the live environment for dynamic data analysis.

On Seqera Cloud, the free tier permits only one running data studio session at a time. To run simultaneous sessions, [contact Seqera][https://seqera.io/contact-us] for a Seqera Cloud Pro license.
...
- A compute environment with sufficient resources. This is highly dependent on the volume of data you wish to process, but it's recommended to have at least 2 CPUs allocated with 8192 MB of memory. See [AWS Batch](../compute-envs/aws-batch.mdx) for more information about compute environment configuration.
- [Data Explorer](./data-explorer.mdx) is enabled.
...
2. Select a container image template from the four options provided (JupyterLab, RStudio Server, Visual Studio Code, or Xpra).
3. Select a compute environment. Currently, only AWS Batch is supported.
4. Mount data using Data Explorer: Select one or more datasets to mount. Datasets are mounted using the [Fusion file system](https://docs.seqera.io/fusion) and are available at `/workspace/data/<dataset>`. Mounted data doesn't need to match the compute environment or region of the cloud provider of the session. However, this might cause increased costs or errors.
5. Optional: Enter CPU and memory allocations. The default values are 2 CPUs and 8192 MB memory (RAM).
6. Select **Add**.

You'll be returned to the Data Studios landing page that displays the list of data studios in your workspace. The session you created will be listed with the status **stopped**. You can inspect the configuration details by selecting the session name.
...
Data Studios currently provides four container image templates: JupyterLab, RStudio Server, Visual Studio Code, and Xpra. The image templates install a very limited number of packages when the session container is built. Users have permission to install additional packages as needed.
...
When adding a new session from an existing session or checkpoint, the following fields are immutable:

- **Data Studio template**
- **Original data studio session and checkpoint**
- **Compute environment**

To add a new session from an existing **stopped** session, complete the following steps:

1. In a workspace, select **Data Studios**.
1. From the list of sessions in your workspace, select the three dots next to the status message for the session you want to start, then select **Start as new**. The **Add data studio** form opens.
1. Optional: In the **Add data studio** form, customize the following fields:
    ...
    - **CPU**
    - **Memory**
1. Select **Add** to add the new session.

Additionally, you can add a new session from any existing checkpoint except the currently running checkpoint. From the detail page, select the **Checkpoints** tab and in the **Actions** column, select **Start as new data studio**. This is useful for tertiary analysis experimentation without impacting the state of the original data studio.
...
Once connected, the session list will update to display the status of **running** and any connected user will have their avatar displayed under the status in both the list of Data Studios and in each session's detail page.
