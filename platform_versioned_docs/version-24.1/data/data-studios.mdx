...
1. For the **Compute & Data** section, complete the following steps:
   ...
   1. Customize the following fields:
      - **Select compute environment**: Currently, only AWS Batch is supported.
...
1. For the **General config** section, complete the following steps:
   ...
   1. Customize the following fields:
      - **Template**: Select a data studio template from the dropdown list. For information about providing your own template with the **Prebuild container image** selection, see TBD.

        :::tip
        If you select the **Prebuild container image** template, you cannot also select **Install Conda packages** as these customization choices are mutually exclusive.
        :::

      - **Data studio name**
      - Optional: **Description**
   1. Optional: Select **Install Conda packages** to enter or upload a list of Conda packages to include with the data studio. For information on the syntax for specifying the Conda packages, see TBD.
   1. Select **Next**.

1. For the **Summary** section, complete the following steps:
   1. Ensure that the specified configuration is correct.
   1. Select **Add**, and then select from the following choices:
      - **Add only**: Save the data studio for future use.
      - **Add and start**: Save and immediately start the data studio.
...
Data Studios currently provides four container image templates: JupyterLab, RStudio Server, Visual Studio Code, and Xpra. The image templates install a very limited number of packages when the data studio session container is built. Users have permission to install additional packages as needed.
...
To add a new data studio from an existing **stopped** data studio, complete the steps described in [Add a data studio](#add-a-data-studio).
