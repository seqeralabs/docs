...
### 24.2.0.cycle21 - 14 August 2024

- Feature: Add global Nextflow configs in compute environments
- Feature: Added flexibility for pipeline names in workspaces
- Feature: Added tags propagation to launch template
- Feature: Add backend and UI to include managed identities into the configuration when using a manual Azure CE
- Feature: Implement custom launch container logic
- Fix: Improve workflow launch L&F
- Fix: Allow changing the workDir when launching a pipeline
- Fix: Properly handle special characters in prompt modal confirmation text regex
- Bump nextflow 24.04 + codecommit 0.2.1
