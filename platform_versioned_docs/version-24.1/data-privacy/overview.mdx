...
- Their email is changed to `none@your-domain` and any runs and runs metadata associated with their account will display that email address going forward.
- Their username is changed to `username-<timestamp of deletion>`.
- All their memberships, including team memberships, are deleted.
- All tokens are deleted.

For Enterprise installations, in addition to the above: 

- All the user's credentials are deleted.
- All the user's compute environments are deleted.
- All actions created by the user are deleted.
