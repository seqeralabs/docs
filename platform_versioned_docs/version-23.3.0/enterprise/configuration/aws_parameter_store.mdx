...
Store Seqera configuration values as individual parameters in the AWS Parameter Store. Values must be namespaced to avoid collisions if multiple Seqera instances exist in the same AWS Account (e.g., Dev / Staging / Prod). Seqera uses `tower-app` by default, but this can be modified via the `tower.application.name` key in the `tower.yml` file.
