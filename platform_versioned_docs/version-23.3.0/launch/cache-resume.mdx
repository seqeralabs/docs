...
Kubernetes compute environments do not use cloud cache by default. To specify a cloud storage cache directory, include the cloud cache path and necessary credentials for that location in your Nextflow configuration. **This is not recommended for production environments**.
   ...
   <details>
      ...
      1. See [these instructions](../compute-envs/google-cloud-batch.md#iam) to set up IAM and create a JSON key file for the custom service account with permissions to your Google Cloud storage account. 
      ...
      3. Add the following to the **Nextflow Config file** field when you [launch](../launch/launchpad.md#launch-form) your pipeline:
