...
- NF Launcher image now pulls by default from `cr.seqera.io` public registry instead of quay.
...
- ECS container timeout configuration now defaults to 10 minutes (`ECS_CONTAINER_CREATE_TIMEOUT=10m,ECS_CONTAINER_STOP_TIMEOUT=10m,ECS_MANIFEST_PULL_TIMEOUT=10m`).
