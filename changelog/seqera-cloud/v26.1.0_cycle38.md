...
- Fixed an issue where custom pipeline info fetch error messages were incorrectly removed when selecting the same value again in pipeline, revision, or main script fields.
- Fixed an issue where empty optimization buttons were displayed on the Launchpad when optimization status was not set.
- Prevented conda Studios from auto-starting when the organization exceeds Studio session limits.
