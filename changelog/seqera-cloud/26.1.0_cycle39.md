...
- Added `pipelineSchemaId` to launch responses and workflow launch requests.
- Hide "Publish version" option from run actions menu for workflows launched with shared pipeline.
- Added `pipelineSchemaId` support to version hashing logic.
- Support for nested non-versionable schema parameters in version hashing.
- Added operation to delete from workspace by userId.
- Support keywords to search pipeline versions by name and hash.
- Make count query optional in `ListWorkflow` endpoint.
...
- Prevented LazyInitializationException in interruptStudioForNoStats.
