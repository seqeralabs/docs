...
- **Studios**
  - Added Studio session lifespan setting. This feature allows you to set a lifespan for a session, after which the session is stopped automatically.
...
- Mark long-running jobs with a warning: This warning displays when a workflow has been in the `RUNNING` state for more than 3 hours and no single task moved from one state to the next.
