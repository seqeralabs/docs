...
### Data Studios
 ...
 - Data Studios now supports [custom environments](https://docs.seqera.io/platform-enterprise/24.2/data_studios/custom-envs):
    ...
    - Audit log now includes custom environment build events. The events `data_studio_session_build_started` and `data_studio_session_build_failed` appear in the audit log events table.
...
- Added NVMe support.
- EFS volume mounting now [supported](https://docs.seqera.io/platform-enterprise/24.2/data_studios/overview#limitations).
...
- Manual Azure Batch compute environments now support authentication using [Microsoft Entra service principals](https://docs.seqera.io/platform-enterprise/24.2/compute-envs/azure-batch#credentials). This provides a secure alternative to long-lived access keys, as service principals enable role-based access control with more precise permissions.
...
- The Google Life Sciences API will be deprecated in June 2025. Seqera Platform UI now displays a Google Life Sciences API deprecation notice.
- AWS Batch launch templates created by Batch Forge now include tag propagation, which propagates resource labels to storage volumes (as well as instances which are the default).

### Credentials

- GitLab credentials no longer require the Password field, as public repositories only require an access token. Note that private repositories will require a credential with both a password and access token.
...
- Refactored now reports file streaming over Agent connections directly to the browser.
...
- Added ECS agent configuration variables to set the timeout for several container actions:
    - `ECS_CONTAINER_CREATE_TIMEOUT=10m`: Timeout for creating a container.
    - `ECS_CONTAINER_STOP_TIMEOUT=10m`: Timeout for stopping a container. Increase to prevent forcibly killing Nextflow and leave running jobs.
...
- Bumped nf-launcher:j17-24.10.2.
...
- Updated `lastUsed` on compute environment when creating a job for a Data Studios session.

### Launchpad

- Fixed input form view for private GitHub repositories using pipeline Quicklaunch.
- Fixed an issue where workspace secrets were not selected by default when using pipeline Quicklaunch.
- Fixed incorrect role access in new launch form which allowed users with "Maintain" role and lower to edit resource labels when launching.
...
- Fixed NF schema nested object values lost when switching to input form view. Removed processing of unsupported JSON schema types inside pipeline input form components.
...
- Fixed issue where searching for task tags using underscores did not return the expected values.
- Fixed report access issue for Nextflow CLI runs with `TOWER_CONTENT_URL` set.

### Compute environments

- Allow resume with compatible compute environments when using Tower Agent and `$TW_AGENT_WORKDIR` as working directory.
- Added `managedIdentityId` to the API response payload when describing a compute environment.

### Data Explorer

- Enabled task working directory navigation using Data Explorer in personal workspaces.
...
- Fixed Entra and `javax.mail` transitive dependency problem.

### Infrastructure

- Improved performance of queries reported as intensive on the RDS database. Added an index to the `tw_workflow(complete)` column, which improves the performance of the query that finds workflows with missing progress. Added an index to the `tw_workflow(status)` column conditionally; the index already exists in the `cloud.seqera.io` database, but did not previously appear in the database migration scripts.
...
- Replaced `/redisson/*` references in AWS Parameter Store entries with `TOWER_REDIS_*`.
- Replaced `redisson.*` references in `tower.yml` with `TOWER_REDIS_*`.
