---
title: Seqera Enterprise v23.3_major
date: 2023-10-10
...
Login redirection logic has changed in version 23.3. Seqera now prepends the [`TOWER_SERVER_URL`](../platform_versioned_docs/version-23.3/configuration/overview.mdx#basic-configuration) (or `tower.serverUrl` in `tower.yml` configuration) to the authentication redirect URL during the login flow. This is useful when your server URL contains a contextual path.
...
:::caution
If you're using AWS Batch, you will need to [configure a custom job definition](../platform_versioned_docs/version-23.3/advanced-topics/custom-launch-container.mdx) and populate the `TOWER_LAUNCH_CONTAINER` with the job definition name instead.
:::
