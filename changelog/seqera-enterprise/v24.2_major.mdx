...
### Nextflow
  ...
  - Adds global Nextflow config: This provides the ability to define the Nextflow config in the CE as a "global" configuration. The configuration is defined in the pipeline config and at launch level and can override the one defined in the CE config.
...
- Implemented custom launch container logic to specify a custom container to run Nextflow when submitting a workflow execution. This is currently only available as an API request.
