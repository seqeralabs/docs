...
The **Use Graviton CPU architecture** option defaults to available during AWS Batch compute environment creation. The `TOWER_ENABLE_ARM64` configuration environment variable is no longer needed to enable ARM64 CPU architecture support.

**Data Explorer default set to `false`**

In previous versions, Data Explorer defaulted to enabled using `TOWER_DATA_EXPLORER_ENABLED=true`. From version 2.4.3, the default is `TOWER_DATA_EXPLORER_ENABLED=false`. If you have upgraded from a previous version and no longer have access to Data Explorer, please check and update your environment variables accordingly.
