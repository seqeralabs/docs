...
Seqera Enterprise v26.1 broadens and scales the platform's capabilities across pipeline launches, AI, cloud infrastructure, data management, and compliance. Co-Scientist is now available for Enterprise deployments. Pipelines now support a GitHub App manifest flow for credential creation and higher throughput than personal access tokens (PATs). Compute environments support descriptions and can be disabled, and Google Batch and Azure Batch have significant stability and scalability improvements. On the cloud provider integration side, AWS, Azure, and GCP each gain deeper credential and identity management including Workload Identity Federation (WIF) for Google and Entra service principal support for Azure Batch. Nextflow data lineage is configurable and attached to pipeline runs and Data Explorer objects. Existing Studios can be edited, and new workspace settings for custom container registry controls and default collaboration modes provide top-down granular management. Datasets now support linked (URL-referenced) remotely-hosted data. Data Explorer adds IGV and Molstar 3D viewer for genomic and structural files, respectively. Audit Logs have a new user interface with on-demand CSV export with configurable max records and richer event context. Finally, the Admin panel exposes real-time usage metrics.
