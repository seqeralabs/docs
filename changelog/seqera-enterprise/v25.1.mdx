...
- **[Private Studios](https://docs.seqera.io/platform-enterprise/25.1/studios/managing#general-config)**: When adding a new Studio, the number of concurrent connections (private vs. all workspace members) can now be defined in **General Config > Collaboration**.
- **[Custom Studio session lifespans](https://docs.seqera.io/platform-enterprise/25.1/studios/managing#modify-session-lifespan-for-all-studios)**: This new feature allows you to set a maximum lifespan for a session, after which time the session is stopped automatically and a checkpoint created, or the session can be extended on-demand.
...
- When a GitHub pipeline is added or updated, if no user-uploaded icon is defined, the icon will default to the one defined the `manifest.icon` Nextflow config; otherwise, it will use the GitHub organization avatar. You can also upload custom icons, which take priority over the auto fetched ones.
