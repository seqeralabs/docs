...
- **Private Studios**: When adding a new Studio, the number of concurrent connections (private vs. all workspace members) can now be defined in **General Config > Collaboration**.
- **Custom Studio session lifespans**: This new feature allows you to set a maximum lifespan for a session, after which time the session is stopped automatically and a checkpoint created, or the session can be extended on-demand.
- **Studios resource labels support**: Workspace maintainer role and above users can manage the resource labels inherited from the compute environment and attached to the Studio. Resource labels attached to each Studio don’t affect the default resource labels associated with the compute environment.
...
When a GitHub pipeline is added or updated, if no user-uploaded icon is defined, the icon will default to the one defined the `manifest.icon` Nextflow config; otherwise, it will use the GitHub organization avatar.
...
- Fixed an issue where the new Launch Form did not select the primary compute environment by default; now a primary compute environment is selected during quick launch.

## Breaking changes and warnings

### OIDC Secrets injection modifications

The `auth-oidc-secrets` Micronaut environment has now been replaced with `oidc-token-import` naming. If you were using this configuration, the change should be applied in between migration. If you were activating the feature using the `TOWER_OIDC_TOKEN_IMPORT` environment variable then no changes are needed on your side.
...
From 24.2 of Seqera Enterprise: 
...
From 24.2 of Seqera Platform, `redisson.*` configuration properties are deprecated. If you have set `redisson.*` properties directly previously, do the following:
...
The property that determines the expiration time of the JWT access token (used for authenticating web sessions and Nextflow-Platform interactions) has changed as of Seqera Platform 24.1:
