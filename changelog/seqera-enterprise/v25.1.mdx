...
The `auth-oidc-secrets` Micronaut environment has now been replaced with `oidc-token-import` naming. If you are using this configuration, the change should be applied to the `MICRONAUT_ENV` environment variable in the manifest, during the migration proces. If you activate the feature using the `TOWER_OIDC_TOKEN_IMPORT` environment variable then no changes are needed.
