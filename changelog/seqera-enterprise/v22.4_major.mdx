---
title: Tower Enterprise v22.4_major
date: 2023-02-03
...
1. The default `nf-launcher` image includes a `curl` command which will fail if your Tower is secured with a private TLS certificate. To mitigate this problem, please see [these instructions](../platform_versioned_docs/version-22.4/configuration/ssl_tls.mdx).
...
!!! warning
    If you're using AWS Batch, you will need to [configure a custom job definition](../platform_versioned_docs/version-22.4/advanced-topics/custom-launch-container.mdx) and populate the `TOWER_LAUNCH_CONTAINER` with the job definition name instead.
