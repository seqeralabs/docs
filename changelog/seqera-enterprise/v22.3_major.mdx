...
Tower now supports applying [resource labels](../platform_versioned_docs/version-22.3/resource-labels/overview.mdx) to compute environments and other Tower elements. This offers a flexible tagging system for annotation and tracking of the cloud services consumed by a run.
...
Wave also enables the use of private container registries in Nextflow — [registry credentials](../platform_versioned_docs/version-22.3/credentials/overview.mdx) stored in Tower are used to authenticate to private container registries with the Wave service.
...
New [compute environment options](./platform_versioned_docs/version-22.3/configuration/overview.mdx#compute-environments) are available:
