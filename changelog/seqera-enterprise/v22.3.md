...
- Exposed label and resource label APIs.
- Raised usable datasets (and dataset versions) per organization to 100 records by default.
- Customize head node resources from the launch/relaunch form.
  - Users (with maintainer permissions) can launch a pipeline in a Tower cloud environment, specifying the head node resources (memory and CPU) from the launch form. This allows you to properly dimension resources and avoid pipeline crashes. This feature is available for AWS, Google Life Sciences, and Kubernetes Compute Environments.
- Extended the **Revision number** field in the launch form to allow a maximum length of 100 characters.
- Improved SSH connector resilience + UGE qstat.
...
- In previous versions, Batch Forge downloaded some assets from S3 bucket named `nf-xpack.s3.eu-west-1.amazonaws.com`. As of version 22.3.x, download those assets from `[https://nf-xpack.seqera.io](https://nf-xpack.seqera.io/)`. Make sure your network policy allows access to the `seqera.io` domain.
...
- Added support for Service Account, VPC and Subnetwork in Google Batch Advanced Settings.
- Added support for AWS Batch `SPOT_PRICE_CAPACITY_OPTIMIZED` allocation strategy.
- Added page size selector and pagination for the tasks table in the workflow details page.
- Added support for Fusion to EKS and GKE platform providers.
- Added support for Secrets for Google Batch and Google LS.
- Improved responsiveness for the workflow runs list page.
- Added support for downloading and previewing bucket files through Data Explorer.
- Adds the possibility to specify a custom base href (useful in a reverse proxy scenario).
- Fixed disabled search bar after getting 0 results for a search in the workflow reports tab.
- Added download as json option for workflow run parameters.
- Decreased audit log lifespan to 90 days.
- Added support for uploading files through Data Explorer.
- Added support for Nextflow cloudcache plugin.
- Added support for navigating workflow and task work directories using Data Explorer.
- Apply new branding to UI and copy.
- Bumped avatar file size limit to 200KB.
- Improved auto-suggested datalink name.
- Disabled upload functionality on public Data Links.
- Fixed tasks total number getting stuck after filtering.
- Previewing text files in Data Explorer now capped at 2000 lines to prevent browser from hanging.
- Enabled instance types selection for dragen queue.
- Fixed display of error messages in pipeline input form.
- Fixed report preview dialog height.
- Adds a new attempt column to the task table.
- Deprecated Fusion V1.
- Added support for selecting pipeline input values using Data Explorer.
- Added a per workspace and global feature toggle for Data Explorer.
- Updated Azure icon (Azure rebranding from May 2021).
- Added support for custom network and subnetwork to Google Cloud Batch compute enviroment.

- Bumped nf-launcher:j17-23.04.3
