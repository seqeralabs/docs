...
Added a new plot type: box plot. It's useful to visualize a distribution when you have a set of values for each sample.
...
Note the difference with the violin plot: the box plot visualizes the distributions of many values within one sample, whereas the violin plot shows the distribution of one metric across many samples.
