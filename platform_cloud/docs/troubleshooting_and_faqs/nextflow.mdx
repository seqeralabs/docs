...
If you're restricted from using public container registries, see Seqera Enterprise release [instructions](https://docs.seqera.io/platform/latest/enterprise/release_notes/23.2.mdx#nextflow-launcher-image) for the specific image to set as the default when invoking pipelines.
...
- For jobs executing in an AWS Batch compute environment, create a [custom job definition](https://docs.seqera.io/platform/latest/enterprise/advanced-topics/custom-launch-container.mdx) which references a different nf-launcher image.
