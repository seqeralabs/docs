...
:::warning
API tokens are tied to users and are different from Access tokens which are tied to resource. While API tokens are supported by Platform, Access tokens are not supported for accessing BitBucket repositories.
