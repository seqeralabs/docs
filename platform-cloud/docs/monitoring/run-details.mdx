...
- [**Settings > Lineage**][workspace-lineage-settings]. A workspace maintainer configures the cloud credentials, region, and (optionally) bucket name where lineage records are stored. Select **Enable lineage by default** to make the launch form lineage toggle default to on for every run launched in the workspace.
...
| **lineage_id** | Lineage ID (LID) of the task's `TaskRun` record. Populated only when lineage tracking is active for the run. Select the LID to navigate to the lineage record. |
...
:::note
The Inputs and Outputs tabs appear only when [lineage tracking is enabled][nextflow-lineage-tutorial] for the run.
:::
...
:::note
Pipeline parameters are displayed on the [Inputs](#inputs) tab.
:::
...
The **Inputs** tab consolidates the pipeline parameters and the input files used by the run.
...
|-------|--------|-------------|
| **Lineage ID** | `LID` | Lineage ID (LID) of the workflow run's `WorkflowRun` record. Select the LID to navigate to the lineage record for the run. |
