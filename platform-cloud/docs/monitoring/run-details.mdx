...
:::tip
Lineage-aware fields and tabs only display data when the run was executed with data lineage enabled. To enable lineage for a run, set `lineage.enabled = true` in your Nextflow configuration, toggle lineage on in the launch form, or enable it as a workspace setting. See [Getting started with data lineage][nextflow-lineage-tutorial] for details.
:::
...
| Condition | Behavior |
|-----------|----------|
| Lineage was not enabled for this run | The Lineage view is hidden. A note reads: *"Lineage tracking was not enabled for this run. Add `lineage.enabled = true` to your Nextflow config to capture lineage data, or toggle lineage on in the launch form."* |
