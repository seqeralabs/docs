...
:::info
If a compute environment is created with Batch Forge, the compute environment will propagate resource labels to the resources generated by the Forge execution at compute environment creation time. See [AWS](#aws) for the list of resources tagged during Batch Forge creation time.
:::
