...
[Seqera Compute](../compute-envs/seqera-compute.mdx) environments consume credits when running pipelines or studio sessions. Credits are consumed for CPU time, memory and storage usage, and network costs. 1 Seqera Compute credit is equivalent to $1 (USD), and resources are charged for at the following rates:
