...
Kubernetes compute environments do not use cloud cache by default. To specify a cloud storage cache directory, include the cloud cache path and necessary credentials for that location in your Nextflow configuration. **This is not recommended for production environments**.
   ...
   <details>
      ...
      3. Add the following to the **Nextflow Config file** field when you [launch](../launch/launchpad#launch-pipelines) your pipeline:
...
Ineligible compute environments appear in the **Compute environment** drop-down but you cannot select them. The drop-down does not state why. If the original compute environment no longer exists, Platform also reports that compute environment as unavailable. That notice concerns only the missing compute environment ID. It does not indicate a work directory mismatch.
