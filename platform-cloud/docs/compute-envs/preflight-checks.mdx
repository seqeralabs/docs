...
- Tower Agent is reachable from Seqera Platform. See [Tower Agent](../getting-started/tower-agent) for installation and startup instructions.

## Validation process

Seqera Platform runs three tiers of validation:
...
Seqera Platform makes a live call to the cloud provider and updates the credential status immediately. If the check passes, the credential returns to **AVAILABLE**.
...
Seqera Platform runs pre-flight checks and updates the compute environment status immediately. If all checks pass, the compute environment returns to **AVAILABLE**.
