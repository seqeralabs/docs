...
Pre-flight checks validate that a compute environment is usable before and at the point of pipeline launch. They run at three points, and the checks differ at each:

- When you create or update a compute environment, Platform verifies the status recorded for the selected credential.
...
Problems appear before pipeline submission rather than mid-run. Pre-flight checks only flag conditions that would block a pipeline launch.
...
A transient probe failure, such as a network interruption or provider throttling, does not mark the credential `INVALID`. Platform retries the credential with exponential backoff. Some failures indicate the credential can never validate, such as an Azure storage or Batch account hostname that no longer resolves. After 10 consecutive failures of this kind, Platform marks the credential `INVALID`.
...
These errors are returned immediately when you create a compute environment, or update one to use different credentials, and the selected credential cannot be used. No changes are saved. On update, only the newly selected credential is checked. Other edits to the compute environment are unaffected. When the credential record has no stored provider error, the invalid-credentials message omits the `— <provider error>` clause.

| Error | Cause | Resolution |
|---|---|---|
| `The credentials '...' are invalid — <provider error>. Update and validate the credentials, then try again` | The selected credential is marked `INVALID` | Go to **Credentials**, update or rotate the credential, select **Validate**, then create or update the compute environment again |
| `The credentials '...' have been deleted. Select different credentials and try again` | The selected credential has been deleted | Select a different credential or create a new one |
