...
- Tower Agent must be running on the cluster and connected to Platform before any pipeline can launch. This is a persistent process managed by whoever administers the cluster — confirm it is active with your cluster administrator if you are unsure.
- See [Tower Agent](../getting-started/tower-agent) for installation and startup instructions.
...
Runs on a recurring schedule. For each cloud credential (AWS, GCP, Azure) in scope, Platform calls the provider API to verify the credential keys are still accepted — for example, that AWS access keys haven't been rotated or that a GCP service account key hasn't been revoked.

When a credential fails this check, it is marked **INVALID** with the specific provider error recorded in the credential record. This state is then visible to the CE sweep and to any launch that references that credential.

:::note
Platform also checks whether the credential permissions are sufficient to run the pipeline, not just that the keys are valid.
:::
