...
Pre-flight checks validate that a compute environment is usable before and at the point of pipeline launch. They run synchronously when you create or update a compute environment and at launch time, and in the background on a recurring schedule. Problems appear before pipeline submission rather than mid-run. Pre-flight checks only flag conditions that would block a pipeline launch.
