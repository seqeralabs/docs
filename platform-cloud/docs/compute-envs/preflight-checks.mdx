...
These errors are returned immediately when you create a compute environment, or update one to use different credentials, and the selected credential cannot be used. No changes are saved. When the credential record has no stored provider error, the invalid-credentials message omits the `— <provider error>` clause.
