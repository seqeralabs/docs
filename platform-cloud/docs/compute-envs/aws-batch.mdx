...
Permissions can be attached directly to an [IAM user](#iam-user-creation), or to an [IAM role](#iam-role-based-creation-creation) that the IAM user can assume when accessing AWS resources.
...
1. Under **Permission options**, select **Attach policies directly**, then search for and select the policy created above, and select **Next**.
  * If you want to use an external ID with a key, add the following permission policy to a user. Edit the AWS principal with the ARN of Seqera's platform role. Seqera Platform will need the access and secret key values, as well as the ARN in order to generate an external ID.
   ...
   ```json
   {
     ...
     "Resource": "arn:aws:iam::161471496260:role/SeqeraPlatformCloudAccessRole",
     "Condition": {
       "StringEquals": {
...
```
:::note
The External ID value must be updated with the value generated by Seqera when you generate an external ID in Seqera Platform
:::
...
Seqera automates the configuration of an [AWS Batch](https://aws.amazon.com/batch/) compute environment and the queues required to deploy Nextflow pipelines. After your IAM User or Role and S3 bucket have been set up have been set up, create a new **AWS Batch** compute environment in Seqera.
