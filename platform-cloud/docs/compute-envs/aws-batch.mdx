...
1. Under **Permission options**, select **Attach policies directly**, then search for and select the policy created above, and select **Next**.
  * If you want to use an external ID with a key, add the following permission policy to a user. Seqera Platform will need the access and secret key values, as well as the ARN in order to generate an external ID.
...
1. If you prefer to use an external ID with , create a role with the following permission policy (edit the AWS principal with the ARN of Seqera's platform role) and grant it the necessary permissions:
...
:::note
Seqera Platform generates the `External ID` value during AWS credential creation. You will need to paste the exact External ID value into the trust policy after creating the credential in Seqera Platform.
:::
