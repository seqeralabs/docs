...
1. Optionally, if you want to use an ARN and External ID with key based access, add the following to the user's Permission policy. This will allow the role to manage batch resources
   ```json
   {
...
1. On the **Permissions** page, search for and select the policy created in the [IAM user creation](#create-the-iam-policy) section, then select **Next**.
...
The External ID is generated by Seqera when you save your credentials. Complete the following steps to finalize the trust policy:
  ...
  2. Return to the IAM role's trust policy in AWS and replace the <ExternalId> placeholder with the generated value. 
:::
