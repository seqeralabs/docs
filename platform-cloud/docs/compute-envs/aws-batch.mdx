...
1. Under **Permission options**, select **Attach policies directly**, then search for and select the policy created above, and select **Next**.
  * If you want to use an external ID with a key, add the following trust policy to a user. Edit the AWS principal with the ARN of Seqera's platform role. Seqera Platform will need the access and secret key values, as well as the ARN in order to generate an external ID.
...
:::note
The External ID placeholder value must be updated with the value generated by Seqera when you generate an external ID in Seqera Platform. The ID is generated upon credential creation 
:::
