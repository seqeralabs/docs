...
:::info[Preview]
Seqera Intelligent Compute is in preview. Seqera must enable it for your workspace before you can use it. Contact your account manager to request access.
:::
...
| **Backend strategy** | `ECS`, `EC2` | `ECS` | Task execution backend. `ECS` delegates execution to AWS ECS (default). `EC2` runs tasks directly on EC2 instances and enables warm pool support. |
