...
Unlike the standard AWS Cloud compute environment, which runs a pipeline on a single EC2 instance with a local executor, Intelligent Compute provisions and manages a multi-node ECS cluster. This allows pipelines to scale beyond a single instance while preserving fast startup times.
