...
### Attach the Intelligent Compute policy

Intelligent Compute requires an AWS Cloud compute environment, so the AWS Cloud policy is already attached to your IAM user or role. You only need to add the Intelligent Compute policy:

1. Open the [AWS IAM console](https://console.aws.amazon.com/iam).
1. Select **Policies** under **Access management**, then select **Create policy**.
1. Select the **JSON** tab, paste the Intelligent Compute policy JSON, then select **Next**.
1. Enter a name (for example, `SeqeraIntelligentComputePolicy`), then select **Create policy**.
