...
For full setup instructions, see [Intelligent Compute](./intelligent-compute.mdx).
...
Seqera Intelligent Compute requires **two IAM policies** attached to the same IAM user or role:
...
Both policies must be attached to the same IAM user or role that Seqera uses to access your AWS account.
...
Some statements in the Intelligent Compute policy are conditional and can be omitted depending on your deployment:
...
If you are also enabling Seqera Intelligent Compute, repeat these steps twice — once for each policy in the [Seqera Intelligent Compute permissions](#seqera-intelligent-compute-permissions) section.
