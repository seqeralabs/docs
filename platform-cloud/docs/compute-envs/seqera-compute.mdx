...
Seqera Compute enables you to seamlessly run pipelines and Studios in Seqera Cloud in an automatically provisioned compute environment on AWS. Seqera autonomously manages all underlying resources, including AWS accounts, credentials, roles, AWS Batch compute environments, and S3 storage buckets, requiring minimal user configuration. Using prepaid Seqera Compute credits enables granular control and visibility of the compute spend in each of your organization's workspaces.
