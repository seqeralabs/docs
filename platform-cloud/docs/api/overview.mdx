...
<p>
  <Button to="https://cloud.seqera.io/openapi/index.html" className="mr-3" blue medium alt>View API reference</Button>
  <Button to="https://cloud.seqera.io/openapi/seqera-api-latest.yml" secondary medium alt>Download API schema</Button>
</p>
...
:::note
The API uses the [semantic versioning convention](https://semver.org/) (major.minor.patch). In the event that a breaking change is introduced in future versions, we will publish guidance on the v1 support schedule and steps to mitigate disruption to your production environment.

- Adding new API endpoints, new HTTP methods to existing endpoints, optional request parameters (body fields, query parameters, headers), or response fields
- Expanding accepted input formats or value ranges (enums, string constants, ...)
...
Both the [Seqera Platform CLI](../cli/overview) and [seqerakit](https://github.com/seqeralabs/seqera-kit) are built on the Platform API and provide a programmatic automation layer for operations.
