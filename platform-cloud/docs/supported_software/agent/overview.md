...
- Outbound HTTPS access from that node to `api.cloud.seqera.io`.
...
| `-u`, `--url=<url>` | `https://api.cloud.seqera.io` | Seqera API endpoint URL. If not set, `TOWER_API_ENDPOINT` is used. |
