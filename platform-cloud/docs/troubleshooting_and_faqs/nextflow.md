...
To force the Seqera-invoked job to use your `nextflow.config` value, add the configuration setting in the workspace Launch screen's [**Nextflow config file** field](../launch/launchpad.md). For the example above, you would add `aws.client.uploadChunkSize = 209715200 // 200 MB`.
...
Fusion v1 has a limitation which causes tasks that run for less than 60 seconds to fail as the output file generated by the task is not yet detected by Nextflow. This is a limitation inherited from a Goofys driver used by the Fusion v1 implementation. [Fusion v2](../supported_software/fusion/fusion.md) resolves this issue.
...
- For jobs executing in an AWS Batch compute environment, create a [custom job definition](https://docs.seqera.io/platform-enterprise/25.1/enterprise/advanced-topics/custom-launch-container) which references a different nf-launcher image.
