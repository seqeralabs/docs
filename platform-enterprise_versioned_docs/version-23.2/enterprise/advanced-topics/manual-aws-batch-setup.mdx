...
1. Select **Amazon EC2** as the compute environment configuration.
    :::note
    AWS Batch compute environments created with [Batch Forge](../../compute-envs/aws-batch#batch-forge) support using Fargate for the head job, but manual compute environments must use EC2.
    :::
