...
If you were using Studios prior to GA (v25.1) please review the `tower.env` file and add the `TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_TOOL` variable. The configuration should be as follows: 

```
TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_STATUS: 'recommended'
TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_REPOSITORY: 'public.cr.seqera.io/platform/data-studio-<template>:4.2.5-0.8'
TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_ICON: '<template_name>'
TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_TOOL: '<template_name>' 
TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_PORT: '8080'
```

You can also check the current template configuration using `https://towerurl/api/studios/templates?workspaceId=<anyworkspaceId>`. The response should include the `TOOL` configuration which should be `jupyter`/`vscode`/`rstudio`/`xpra` (not `custom`).
...
If you were using Studios prior to GA (v25.1) please review the `configmap.yaml` file and add the `TOWER_DATA_STUDIO_TEMPLATES_<TEMPLATE_NAME>_TOOL` variable. The configuration should be as follows: 
