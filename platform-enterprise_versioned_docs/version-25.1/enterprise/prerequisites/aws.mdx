...
Run the Seqera container with [Docker](../docker-compose.md) on an AWS EC2 instance, or with [Kubernetes](../kubernetes.md) on an Amazon EKS cluster. You must satisfy the requirements for your installation target:
...
- **(Optional) AWS Parameter Store**: Store sensitive Seqera configuration values as SecureString [AWS Parameter Store](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-parameter-store.html) parameters. See [AWS Parameter Store configuration](../configuration/aws_parameter_store.md) for instructions. This is recommended for production environments.
...
You can retrieve Seqera Enterprise configuration values remotely from the AWS Parameter Store. See [AWS Parameter Store configuration](../configuration/aws_parameter_store.md) for instructions.
