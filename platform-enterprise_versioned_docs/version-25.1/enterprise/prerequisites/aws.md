...
:::info 
The following recommendations are minimum system requirements to successfully deploy Seqera Enterprise. Your deployment may require larger database and compute instances to scale to production, depending on the size and number of parallel workflows you expect to run. 
:::
...
Create an instance with at least these attributes:
...
You can retrieve Seqera Enterprise configuration values remotely from the AWS Parameter Store. See [AWS Parameter Store configuration](../configuration/aws_parameter_store.mdx) for instructions. 
