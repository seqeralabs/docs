...
5. Set a Studio name and optional description.
6. Set any environment variables supported by the example.
7. Select **Add and start** to create and launch the Studio.
...
You can use environment variables or data-links to make files available to your custom Studio:

#### Environment variables {#env-vars}

Some examples define environment variables that accept cloud storage paths (such as `s3://bucket/path/to/file.csv`). When you create a Studio, set the value of these variables in the **More settings** section. The container translates the cloud path to the corresponding local path at `/workspace/data/` automatically.
...
6. Select the credentials used to access this directory.
7. Add an optional description and then select **Add**.
