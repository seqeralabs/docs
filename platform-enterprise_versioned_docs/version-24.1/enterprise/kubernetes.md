...
:::warning
The in-cluster Redis manifests below are provided for convenience only. They pull from Docker Hub's public registry (rate limits apply without authentication). For production deployments, use a managed Redis service or an [official Redis installation source](https://redis.io/docs/latest/operate/oss_and_stack/install/).
:::
