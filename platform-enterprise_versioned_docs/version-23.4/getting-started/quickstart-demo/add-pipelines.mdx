...
:::tip
To launch pipelines directly with CLI tools, select the **Launch Pipeline** tab to grab commands for Nextflow, [Seqera Platform CLI](./automation), and [nf-core/tools](https://nf-co.re/docs/nf-core-tools):
...
- (_Optional_) **Labels**: Categorize the pipeline according to arbitrary criteria (such as reference genome version) that may help workspace participants to select the appropriate pipeline for their analysis.
...
- **Revision number**: Platform will search all of the available tags and branches in the provided pipeline repository and render a dropdown to select the appropriate version.
  ...
  :::
- (_Optional_) **Config profiles**: `test`
  - Select a predefined profile for the Nextflow pipeline.
    ...
    :::
- (_Optional_) **Pipeline parameters**:
