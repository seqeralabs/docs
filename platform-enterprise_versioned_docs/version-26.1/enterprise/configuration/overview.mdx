...
<TabItem value="tower.yml" label="tower.yml" default>
    ...
    In a [Helm chart](../platform-helm) deployment, set any of the `tower.yml` values on this page in the `platform.YAMLConfigFileContent` field of your values file. The chart writes this content to the `tower.yml` file that Platform reads:
