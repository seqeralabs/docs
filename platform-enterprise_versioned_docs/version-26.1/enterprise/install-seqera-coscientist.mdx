...
- If you use AWS Bedrock, an AWS account with model access, the IAM permissions the agent backend pods need, and an AgentCore runtime if you enable sandboxed sessions. See [Set up Amazon Bedrock](../co-scientist/bedrock-setup.md).
- If you use direct Anthropic API access, an Anthropic API key stored in a Kubernetes Secret.

:::

## Generate the token encryption key and store secrets
...
Seqera publishes MCP server images for 1.4.3 and later only to `cr.seqera.io/enterprise/mcp/server`. The 26.1 charts default the MCP image repository to `ai/mcp/server`, which holds releases up to 1.4.2 only. Seqera does not patch the 26.1 charts to change this default. If your chart deploys MCP 1.4.3 or later, override the repository:
...
The chart sets no default registry. Set `global.imageRegistry` to the registry you mirror Seqera images to, or to `cr.seqera.io` if your cluster pulls directly. If you mirror images, mirror `enterprise/mcp/server`.
