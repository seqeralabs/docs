...
- If you use AWS Bedrock, a configured AWS account: model access, the IAM permissions the agent backend pods need, and an AgentCore runtime if you enable sandboxed sessions. See [Bedrock setup](../co-scientist/bedrock-setup.md).
...
MCP server images from 1.4.3 are published only to `cr.seqera.io/enterprise/mcp/server`. The 26.1 charts default the MCP image repository to `ai/mcp/server`, which serves releases up to 1.4.2 only, and the chart is not patched retroactively. If your chart deploys MCP 1.4.3 or later, override the repository:

```yaml
mcp:
  image:
    repository: enterprise/mcp/server
```

The chart sets no default registry. Set `global.imageRegistry` to the registry you copy Seqera images into, or to `cr.seqera.io` if your cluster pulls directly. If you mirror images, mirror `enterprise/mcp/server`.
...
<TabItem value="npm package" label="npm package" default>
...
- [Co-Scientist in the Seqera CLI](../co-scientist/index.md): Co-Scientist documentation.
