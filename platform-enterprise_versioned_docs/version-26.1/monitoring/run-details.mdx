...
The **Outputs** tab shows files produced by the run. It contains up to two sub-tabs depending on how the pipeline is configured:

- **Reports** — Named report files configured via `tower.yml`. Available for all pipeline runs where reports are configured. See [Reports](../reports/overview).
- **Pipeline outputs** — All output files declared by the pipeline's [workflow output syntax][nextflow-workflow-outputs], with lineage IDs and labels linking each file back to the run that produced it. Requires [data lineage enabled][workspace-lineage-settings] and a pipeline using the [Nextflow workflow output syntax][nextflow-workflow-outputs] (Nextflow 24.10.0 or later).
...
The **Pipeline outputs** sub-tab lists all `FileOutput` records linked to the workflow run:

| Column | Description |
|--------|-------------|
| **Output Name** | Name of the output file. Select to open the file in [Data Explorer](../data/data-explorer). |
| **File Path** | Full path to the output file in storage. |
| **Type** | Lineage type, such as `Path`. |
| **Lineage ID** | Lineage ID (LID) of the `FileOutput` record. Select to navigate to the lineage record. |
| **Lineage Labels** | Lineage labels assigned to the output. Each label is a clickable link to the lineage record for that label. |

:::note
The **Pipeline outputs** sub-tab requires both:

1. [Data lineage enabled][workspace-lineage-settings] for the workspace or the individual run. See [Data lineage](../data/data-lineage).
2. A pipeline that uses the [Nextflow workflow output syntax][nextflow-workflow-outputs] (Nextflow 24.10.0 or later). Pipelines that publish files using only `publishDir` without a workflow-level `output:` block do not generate `FileOutput` lineage records.
:::
