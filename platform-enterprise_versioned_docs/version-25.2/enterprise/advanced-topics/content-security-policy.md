...
HTTP security headers are an important part of a websites' security posture. They protect against different types of attacks including cross-site scripting (XSS), SQL injection, and clickjacking. Object storage is external to Seqera Platform, and read and write access is strictly limited to a selected group of object storage providers. These select providers are explicitly defined in the platform Content Security Policy.
...
If your object storage provider and Seqera Platform deployment share the same subdomain (e.g. `minio.janedoepharma.com` and `platform.janedoepharma.com`) then communication between platform and provider works **without** additional customization. However, if your object storage provider and platform subdomain don't match, the CSP headers need to be customized.
