...
Fusion supports AWS S3, Azure Blob, and Google Cloud Storage.
...
Fusion is not a general purpose file system. It has been designed to optimize the data transfer of Nextflow pipelines by taking advantage of the Nextflow data model.

### I tried Fusion, but I didn't notice any performance improvement. Why?

The computing nodes in your cluster should use NVMe SSD storage or an equivalent technology. Fusion implements an
aggressive caching strategy that requires the use of local scratch storage based on solid-state disks. See your Platform compute environment page for optimal compute instance and storage recommendations: 
