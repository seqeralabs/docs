...
| `fusion_chunk_errors_total`            | Counter   | Absent, except for `error_type="canceled"`.                        | Counts under `stall`, `timeout`, `transient`, or `exhausted` are real failures. Exclude `canceled`, which marks a normal caller interruption. |
