...
- **Nextflow config**: Set AWS Batch max spot attempts and the custom Fusion container URL received from Seqera [shown below](#enable-snapshot-via-nextflow-config)
...
See [AWS Batch](./guide/aws-batch.mdx#nextflow-cli) for standard Fusion configuration in a Nextflow CLI execution. To use Snapshots, also configure the following: 
...
- To use Fusion Snapshots in a Nextflow CLI execution with the AWS Batch executor, set the following values in your pipeline `nextflow.conf` file:
