...
- `maxSpotAttempts` must be a value higher than `0` to enable automatic retries of tasks assigned to instances which are terminated.
- `fusion.containerConfigUrl` is the URL to the Fusion Snapshots binary software which performs the snapshot and restore operations.
