...
1. Add the following to your `nextflow.config` file:
    ...
    - `<PLATFORM_ACCESS_TOKEN>` with your Platform access token.
    - `<GKE_CLUSTER_CONTEXT>` with your Kubernetes configuration context name.
...
1. Run the pipeline with the Nextflow run command:
    ...
    - `<GCS_BUCKET>` with your Google Cloud Storage bucket.
