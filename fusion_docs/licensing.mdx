...
Paid Seqera Platform Cloud user / Enterprise user / Academic user (limits with OR)

- Unlimited tasks in a month
- Unlimited bandwidth read in a month
- Short lived certificates (per run)
...
License checks occur when a pipeline run starts. If Fusion is unable to validate the license provided by Nextflow, the pipeline will exit with an error message. If the license is valid but expires between retrieval and validation by Fusion, Nextflow will stop and an error code will be displayed.
