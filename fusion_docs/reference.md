...
| `fusion.privileged`               | _New in version 23.10.0_<br /> If `true`, enables privileged containers. Non-privileged use is supported only on Kubernetes with the `k8s-fuse-plugin` or a similar FUSE device plugin. This option does not apply to Seqera Intelligent Compute. Default: `true`. |
